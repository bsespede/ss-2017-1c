...
public class BeemanIntegrator implements Integrator{
	...
	public void move(final Particle particle, final List<Particle> particles, final Walls walls, final double dt) {
		...
		final Vector2d totalForce = Force.getTotalForce(particle, particles, walls);
        ...
        final Vector2d prevAcceleration = Force.getTotalForce(previous, particles, walls).scale(1.0 / particle.getMass());
        ...
        final Vector2d nextAcceleration = Force.getTotalForce(particle, particles, walls).scale(1.0 / particle.getMass());
